import numpy as np

from src.arrayUtility.bmColReshape import bmColReshape
from src.image123.bmImGrid import bmImGrid

def bmImReg_transform_to_deformField(argTransform, n_u, X, Y, Z):
    imDim = np.size(n_u.ravel())
    n_u = n_u.ravel().reshape(-1, 1)
    X, Y, Z = bmImGrid(n_u, X, Y, Z)

    if argTransform.class_type == 'bmImReg_translationTransform':
        t = argTransform.t.ravel()

        if imDim == 2:
            vx = np.ones((n_u.shape[0], 1)) * t[0]
            vy = np.ones((n_u.shape[0], 1)) * t[1]
            v = np.hstack((vx, vy))

        elif imDim == 3:
            vx = np.ones((n_u.shape[0], 1)) * t[0]
            vy = np.ones((n_u.shape[0], 1)) * t[1]
            vz = np.ones((n_u.shape[0], 1)) * t[2]
            v = np.hstack((vx, vy, vz))

    elif argTransform.class_type == 'bmImReg_solidTransform':
        X, Y, Z = bmImGrid(n_u, X, Y, Z)

        t       = argTransform.t.ravel()
        c_ref   = argTransform.c_ref.ravel()
        R       = argTransform.R

        if imDim == 2:
            r = np.hstack((X.ravel().reshape(-1, 1), Y.ravel().reshape(-1, 
1)))
            r = r - np.tile(c_ref.reshape(-1, 1), (1, n_u.shape[0]))
            v = R @ r - r + np.tile(t.reshape(-1, 1), (1, n_u.shape[0]))
            v = np.hstack((v[:, 0], v[:, 1]))

        elif imDim == 3:
            r = np.hstack((X.ravel().reshape(-1, 1), Y.ravel().reshape(-1, 
1), Z.ravel().reshape(-1, 1)))
            r = r - np.tile(c_ref.reshape(-1, 1), (1, n_u.shape[0]))
            v = R @ r - r + np.tile(t.reshape(-1, 1), (1, n_u.shape[0]))
            v = np.hstack((v[:, 0], v[:, 1], v[:, 2]))

    elif argTransform.class_type == 'bmImReg_directTransform':
        v = bmColReshape(argTransform.v, n_u)

    return v
